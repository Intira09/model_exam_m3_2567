...
with open('/content/drive/MyDrive/20 year/thai_loanwords_new_update.json', 'r', encoding='utf-8') as f:
...
# --- โหลดไฟล์คำตอบ ---
df = pd.read_csv("/content/drive/MyDrive/20 year/คำตอบนักเรียน + คะแนนครูสองคน ไฟนอล(Sheet1) (3).csv")
...
for idx, row in df.iterrows():
    ...
    row_data = {
        "index": idx,
        "student_answer_1": answer_text,
        "cosine_similarity": result.get("cosine_similarity", None),
        "ใจความที่ 1": mind_score.get("ใจความที่ 1", 0),
        "ใจความที่ 2": mind_score.get("ใจความที่ 2", 0),
        "ใจความที่ 3": mind_score.get("ใจความที่ 3", 0),
        "ใจความที่ 4": mind_score.get("ใจความที่ 4", 0),
        "คะแนนรวมใจความ": mind_score.get("คะแนนรวมใจความ ", 0),
        "คะแนนการสะกดคำ": result.get("คะแนนการสะกดคำ (1 คะแนน)", 0),
        "เงื่อนไขที่ผิด": "; ".join(result.get("เงื่อนไขที่ผิด", [])),
        "คะแนนรวมทั้งหมด": result.get("คะแนนรวมทั้งหมด", 0)
    }

    rows.append(row_data)

    # --- แสดงผลลัพธ์ระหว่างรัน ---
    print("=" * 60)
    print(f"แถวที่ {idx}")
    print(f"คำตอบ: {answer_text}")
    print(f"cosine_similarity: {row_data['cosine_similarity']:.3f}")
    print(f"ใจความ -> 1:{row_data['ใจความที่ 1']}  2:{row_data['ใจความที่ 2']}  3:{row_data['ใจความที่ 3']}  4:{row_data['ใจความที่ 4']}")
    print(f"คะแนนรวมใจความ: {row_data['คะแนนรวมใจความ']}")
    print(f"คะแนนการสะกดคำ: {row_data['คะแนนการสะกดคำ']}")
    print(f"เงื่อนไขที่ผิด: {row_data['เงื่อนไขที่ผิด']}")
    print(f"คะแนนรวมทั้งหมด: {row_data['คะแนนรวมทั้งหมด']}")
    print("=" * 60)
...
# บันทึกเป็น CSV
out_df.to_csv("/content/drive/MyDrive/20 year/results.csv", index=False, encoding="utf-8-sig")

print("✅ ประเมินเสร็จแล้ว เก็บผลใน evaluation_results.csv")
